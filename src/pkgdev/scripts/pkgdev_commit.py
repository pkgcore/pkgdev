...
class PkgSummary(ChangeSummary):
    ...
    def add(self):
        ...
        if len(self.existing) == len(self.changes):
            ...
            pkgs = sorted(
                x for x in self.repo.match(atom.unversioned_atom) if x <= atom
            )
            try:
            ...
            if old_pkg.eapi in new_pkg.eapi.inherits[1:]:
                ...
                if updated := sorted(watch_vars & updated_vars):
                    ...
                    use_expand = {
                        ...
                        for use, _ in self.repo.use_expand_desc[
                            use_expand_mapping[target]
                        ]
                    }
                    if target in array_targets:
                        ...
                        old = {
                            py_re(m.group("val"))
                            for m in re.finditer(array_re, drop[target])
                        }
                        new = {
                            py_re(m.group("val"))
                            for m in re.finditer(array_re, add[target])
                        }
                    else:
...
class GitChanges(UserDict):
                ...
                if mo := self._ebuild_re.match(path):
                    ...
                    try:
                        ...
                        if status == "R" and (om := self._ebuild_re.match(old_path)):
                            old = atom_cls(
                                f"={om.group('category')}/{om.group('package')}"
                            )
                    ...
                    changes[PkgChange].add(
                        PkgChange(
                            self._repo.location, status, path, atom=atom, ebuild=False
                        )
                    )
            elif mo := self._eclass_re.match(path):
                changes[EclassChange].add(
                    EclassChange(
                        self._repo.location, status, path, name=mo.group("name")
                    )
                )
            else:
                changes[path_components[0]].add(
                    Change(self._repo.location, status, path)
                )
    ...
    def prefix(self):
        ...
        pkg_with_profile_update = len(self.data) == 2 and set(self.data) == {
            PkgChange,
            "profiles",
        }
    ...
    if options.file:
        ...
        if options.message_template:
            ...
            try:
                ...
                commit.error(
                    f"empty message template: {options.message_template.name!r}"
                )
        else:
...
def update_manifests(options, out, err, changes):
    ...
    repo = options.repo
    untracked_ebuild_re = re.compile(
        r"^\?\? (?P<category>[^/]+)/[^/]+/(?P<package>[^/]+)\.ebuild$"
    )

    # update manifests for existing packages
    if atoms := {x.atom.unversioned_atom for x in changes.ebuild_changes}:
        if pkgs := {
            x.versioned_atom for x in repo.itermatch(packages.OrRestriction(*atoms))
        }:
