        ...
        with chdir(repo.location):
            ...
            for opt in ("-b", "--bug"):
                options, _ = tool.parse_args(
                    ["commit", opt, "https://bugs.gentoo.org/2"]
                )
...
class TestPkgdevCommit:
    ...
    def test_message_template(self, capsys, repo, make_git_repo, tmp_path):
        ...
        for i, opt in enumerate(["-M", "--message-template"], 5):
            ...
            assert err.strip().startswith(
                "pkgdev commit: error: empty message template"
            )
    ...
    def test_generated_commit_summaries(self, capsys, repo, make_git_repo):
        ...
        with open(
            pjoin(repo.path, "profiles", "desc", "ruby_targets.desc"), "w"
        ) as file:
        ...
        repo.create_ebuild(
            "cat/pkg-8", use_ruby=" ".join(f"ruby{i}" for i in range(30, 40))
        )
        assert commit() == "cat/pkg: update USE_RUBY support"

        # update array_targets (PYTHON_COMPAT)
        with open(
            pjoin(repo.path, "profiles", "desc", "python_targets.desc"), "w"
        ) as file:
        ...
        # package rename
        shutil.copytree(
            pjoin(git_repo.path, "cat/pkg"), pjoin(git_repo.path, "newcat/pkg")
        )
    ...
    def test_generated_commit_summaries_keywords(
        self, capsys, make_repo, make_git_repo
    ):
        ...
        # stabilize version
        repo.create_ebuild(
            "cat/newpkg-0", keywords=["~amd64", "~arm64", "~ia64", "~x86"]
        )
    ...
    def test_non_gentoo_file_mangling(self, repo, make_git_repo):
        ...
        def commit(args):
            with patch("sys.argv", self.args + args), pytest.raises(
                SystemExit
            ) as excinfo, chdir(git_repo.path):
                self.script()
            assert excinfo.value.code == 0
    ...
    def test_scan(self, capsys, repo, make_git_repo):
        ...
        for i, opt in enumerate(["-s", "--scan"], 1):
            repo.create_ebuild(f"cat/pkg-{i}")
            git_repo.add_all(f"cat/pkg-{i}", commit=False)
            with patch("sys.argv", self.args + [opt]), pytest.raises(
                SystemExit
            ) as excinfo, chdir(git_repo.path):
                self.script()
            assert excinfo.value.code == 0
    ...
    def test_failed_scan(self, capsys, repo, make_git_repo):
        ...
        git_repo.add_all("cat/pkg-1", commit=False)
        with patch("sys.argv", self.args + ["--scan"]), pytest.raises(
            SystemExit
        ) as excinfo, chdir(git_repo.path):
            self.script()
