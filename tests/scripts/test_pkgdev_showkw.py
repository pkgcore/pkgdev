...
class TestPkgdevShowkw:
    ...
    def _run_and_parse(self, capsys, *args):
        with patch(
            "sys.argv", [*self.base_args, "--format", "presto", *args]
    ...
    def test_match(self, capsys, make_repo):
        ...
        with patch(
            "sys.argv", [*self.base_args, "-r", repo.location, "foo/bar"]
        ), pytest.raises(SystemExit) as excinfo:
            self.script()
        assert excinfo.value.code is None
        out, err = capsys.readouterr()
    ...
    def test_match_short_name(self, capsys, make_repo):
        ...
        repo.create_ebuild("foo/bar-0")
        with patch(
            "sys.argv", [*self.base_args, "-r", repo.location, "bar"]
        ), pytest.raises(SystemExit) as excinfo:
            self.script()
        assert excinfo.value.code is None
        out, err = capsys.readouterr()
    ...
    def test_match_cwd_pkg(self, capsys, make_repo):
        repo = self._create_repo(make_repo)
        repo.create_ebuild("foo/bar-0")
        with patch("sys.argv", self.base_args), pytest.raises(
            SystemExit
        ) as excinfo, chdir(repo.location + "/foo/bar"):
            self.script()
        assert excinfo.value.code is None
    ...
    def test_match_specific_arch(self, capsys, make_repo):
        repo = self._create_repo(make_repo)
        repo.create_ebuild("foo/bar-0", keywords=("~amd64", "~ia64", "~mips", "x86"))
        res = self._run_and_parse(
            capsys, "-r", repo.location, "foo/bar", "--arch", "amd64"
        )
        assert set(res.keys()) == {"0"}
    ...
    def test_match_specific_multiple_arch(self, capsys, make_repo):
        ...
        repo.create_ebuild("foo/bar-0", keywords=("~amd64", "~ia64", "~mips", "x86"))
        res = self._run_and_parse(
            capsys, "-r", repo.location, "foo/bar", "--arch", "amd64,mips"
        )
        assert set(res.keys()) == {"0"}
    ...
    def test_correct_keywords_status(self, capsys, make_repo):
        ...
        repo.create_ebuild(
            "foo/bar-2", keywords=("-*", "amd64", "-x86"), eapi=8, slot=2
        )
        res = self._run_and_parse(capsys, "-r", repo.location, "foo/bar")
        assert set(res.keys()) == {"0", "1", "2"}
        assert (
            dict(amd64="+", ia64="~", mips="~", x86="+", slot="0").items()
            <= res["0"].items()
        )
        assert (
            dict(amd64="~", ia64="o", mips="-", x86="~", slot="0").items()
            <= res["1"].items()
        )
        assert (
    ...
    def test_collapse(self, capsys, make_repo, arg, expected):
        ...
        with patch(
            "sys.argv",
            [*self.base_args, "-r", repo.location, "foo/bar", "--collapse", arg],
        ), pytest.raises(SystemExit) as excinfo:
            self.script()
        out, err = capsys.readouterr()
        assert excinfo.value.code is None
